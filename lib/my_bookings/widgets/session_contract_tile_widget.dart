...
class SessionContractTileWidget extends StatelessWidget {
  const SessionContractTileWidget({required this.sessionContract, super.key});
            ...
            children: [
              Row(
                children: [
                  ...
                  if (sessionContract.isPurchased ?? false)
                    Container(
                      margin: EdgeInsets.only(left: 5.w),
                      child: Row(
                        children: [
                          AttendButton(
                            isEnabled: sessionContract.isAttendable ?? false,
                            onPressed: () {},
                          ),
                          if (sessionContract.isAttended ?? false)
                            SizedBox()
                          else
                            Padding(
                              ...
                              child: CancelButton(
                                onPressed: () {
                                  DialogBoxes.showTwoButtonAlertDialog(context,
                                      ...
                                      title: 'Are you sure?',
                                      description:
                                          'Are you sure you want to cancel ${sessionContract.serviceName}?',
                                      confirmText: 'Yes',
                                      dismissText: 'Dismiss', onConfirm: () {
                                    BlocProvider.of<SessionContractsBloc>(
                                            context)
                                        .add(CancelBookedPT(
                                            customerPrivateCoachSessionId:
                                                sessionContract.id));
                                    Navigator.pop(context);
                                  }, onDismiss: () {
                                    Navigator.pop(context);
