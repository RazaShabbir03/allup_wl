...
Future<void> showSessionContractModalSheet(
      ...
      builder: (_) {
        return SafeArea(
          child: Container(
            ...
            child: BrandAdminBottomSheet(
                onSelected: onSelected, sessionContracts: sessionContracts),
          ),
        );
      });
}
...
class BrandAdminBottomSheet extends StatefulWidget {
  const BrandAdminBottomSheet(
      {required this.onSelected, required this.sessionContracts, super.key});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (widget.sessionContracts.isEmpty)
          Padding(
            padding: EdgeInsets.symmetric(horizontal: 20.w, vertical: 20.h),
            child: Text(
              'No session contracts available.',
          ...
          Container(
            ...
            child: ListView.separated(
                padding: EdgeInsets.only(
                    left: 15, right: 15, top: 20.h, bottom: 20.h),
                ...
                shrinkWrap: true,
                itemCount: widget.sessionContracts.length,
                itemBuilder: (context, index) {
                  return InkWell(
                    onTap: () {
                      widget.onSelected(widget.sessionContracts[index]!);
                      Navigator.pop(context);
                    },
                    child: Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 10.0),
                      child: Text(
                        widget.sessionContracts[index]?.service.name ??
                            'Unknown',
                        style: AppTextStyle.rubikStyle(
